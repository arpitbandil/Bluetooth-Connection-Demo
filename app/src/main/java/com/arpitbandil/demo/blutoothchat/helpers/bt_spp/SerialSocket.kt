...
@SuppressLint("MissingPermission") class SerialSocket(private val context: Context, private val device: BluetoothDevice) :
    Runnable {
    ...
    override fun run() { // connect & read
        try {
            socket = device.createRfcommSocketToServiceRecord(Constants.BLUETOOTH_SPP)
            socket?.connect()
